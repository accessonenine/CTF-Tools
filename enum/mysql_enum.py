...
nmap_tool_name = 'NMAP MySQL Enum'
hydra_brute = 'Hydra MySQL Brute Force'


def execute_nmap_mysql_enum(ip_address, port_number):
    command = "nmap -sV -p %s --script=mysql* %s" % (port_number, ip_address)
...
def brute_force(ip_address):
    file_name = './temp/' + ip_address + "_mysql.txt"
    command = 'hydra -t 10 -V -f -L ./resources/common_users.txt -P ./resources/common_passwords.txt mysql://' + ip_address + '>' + file_name
...
def start(ip_address, service):
    ...
    try:
        result = pool.apply_async(execute_nmap_mysql_enum, [ip_address, service.port])
    ...
    try:
        ...
        enum = EnumerationDTO('Check MySQL Vulnerabilities')
        enum.results_output = output
        service.enumeration_list.append(enum)
    except TimeoutError:
